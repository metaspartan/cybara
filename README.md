...
Cybara ships 64 provider definitions in `src/core/providers.ts`, spanning hosted frontier APIs, OAuth-backed coding providers, local runtimes, gateway/proxy providers, and AWS Bedrock. Model lists are discovered dynamically from each provider when supported.
