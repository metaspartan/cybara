...
function networkFailureMessage(baseUrl: string): string {
  ...
  return `Could not reach ${baseUrl}. Open ${baseUrl}/api/health in Safari on this phone to verify the network path. For LAN pairing, make sure the phone is on the same Wi-Fi, Local Network access is allowed, and Windows Firewall allows inbound TCP 4269. For remote pairing, make sure the private mesh or HTTPS tunnel is connected and the gateway URL matches Settings > Gateway > Remote Access.`;
}
