...
export function Connectors() {
  ...
  return (
    <PageLayout
      title="Connectors"
      subtitle="Accounts your agents can work with"
      actions={
        <Button
          variant="ghost"
          size="sm"
    ...
    >
      <div className="mx-auto flex w-full max-w-5xl flex-col gap-5">
                  ...
                  <div className="space-y-4 p-4">
                    ...
                    {status.clientSecretLabel ? (
                      <Input
                        ...
                        onChange={(event) =>
                          patchDraft(status.id, { clientSecret: event.target.value })
                        }
        ...
        <div className="flex items-center gap-3 rounded-lg border border-white/10 bg-white/[0.02] px-4 py-3">
          <KeyRound className="h-5 w-5 shrink-0 text-gray-500" />
          <p className="text-sm text-gray-400">
            Need another service? Add its HTTPS MCP endpoint from More services and authorize it
            there.
          </p>
        </div>
      </div>
