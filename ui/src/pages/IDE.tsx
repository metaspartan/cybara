...
function CodeViewer({
  ...
  useEffect(() => {
    ...
    const handleWindowKeyDown = (event: KeyboardEvent) => {
      if ((event.metaKey || event.ctrlKey) && event.key.toLowerCase() === "g") {
        ...
        lineJumpInputRef.current?.focus();
  ...
  const parseLineJump = (value: string): number | null => {
  ...
  const jumpToLine = useCallback(
    (requestedLine: number) => {
      const sourceText = isEditing ? editContent : content || "";
      ...
      if (isEditing) {
        ...
        const computedLineHeight = Number.parseFloat(
          window.getComputedStyle(textarea).lineHeight || "21"
        );
        const lineHeight = Number.isFinite(computedLineHeight) ? computedLineHeight : 21;
        textarea.scrollTop = Math.max((line - 1) * lineHeight - 64, 0);
        return;
      }
      ...
      const lineNode = container.querySelector<HTMLElement>(`[data-line-number="${line}"]`);
  ...
  const handleLineJumpSubmit = useCallback(
    (event: React.FormEvent<HTMLFormElement>) => {
      event.preventDefault();
      const parsed = parseLineJump(lineJumpInput);
      if (!parsed) return;
      jumpToLine(parsed);
    },
    [lineJumpInput, jumpToLine]
  );
  ...
  const sourceText = isEditing ? editContent : content || "";
  const totalLines = Math.max(1, sourceText.split("\n").length);
    ...
    <div className="flex-1 flex flex-col overflow-hidden">
      <div className="flex items-center justify-between px-4 py-2 border-b border-white/10 bg-white/5">
        <div className="flex items-center gap-2">
          ...
          <span
            className="hidden lg:inline text-xs text-gray-500 max-w-[260px] truncate"
            title={path}
          >
            {path}
          </span>
        ...
        <div className="flex items-center gap-2 text-xs text-gray-500">
          {!isBinary && (
            <form onSubmit={handleLineJumpSubmit} className="flex items-center gap-1">
              <span className="text-[10px] uppercase tracking-wide text-gray-500">Ln</span>
              <input
                ...
                className="w-16 px-1.5 py-0.5 rounded border border-white/10 bg-black/30 text-gray-200 text-xs !outline-none focus:border-indigo-500/50"
                title="Jump to line (Cmd/Ctrl+G)"
              />
            </form>
          )}
      ...
      <div ref={viewContainerRef} className="flex-1 overflow-auto">
        {isEditing ? (
          ...
          <Highlight theme={themes.nightOwl} code={content || ""} language={language}>
            {({ className, style, tokens, getLineProps, getTokenProps }) => (
              <pre
                className={cn(className, "p-4 text-sm overflow-x-auto")}
  ...
  return (
    <div className="px-3 py-2 border-t border-white/10 bg-white/5">
      <div className="flex items-center gap-2 text-xs text-gray-500">
        ...
        <span>LSP:</span>
        {available.length > 0 ? (
          <span className="text-emerald-400">{available.map((l) => l.name).join(", ")}</span>
        ) : (
