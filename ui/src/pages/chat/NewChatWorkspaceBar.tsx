...
export function NewChatWorkspaceBar({
  ...
  return (
    <div className="new-chat-workspace-bar mx-4 flex h-10 min-w-0 items-center gap-3 rounded-t-[18px] border border-b-0 px-3 pb-1 text-[12px]">
      <button
        type="button"
        onClick={onSelectWorkspace}
        disabled={workspaceSaving}
        className="flex min-w-0 items-center gap-1.5 rounded-md px-1 py-1 text-[var(--text-secondary)] transition-colors hover:bg-[var(--surface-hover)] hover:text-[var(--text-primary)] disabled:cursor-not-allowed disabled:opacity-60"
      ...
      {workspaceDir ? (
        <>
          <span className="flex shrink-0 items-center gap-1.5 text-[var(--text-secondary)]">
            <Monitor className="h-3.5 w-3.5 text-[var(--icon-muted)]" />
            Local
          </span>
          <GitBranchSelector
            appearance="inline"
