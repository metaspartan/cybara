...
describe("system prompt reliability guidance", () => {
  ...
  test("includes a concise evidence-backed execution contract", () => {
    ...
    expect(prompt).toContain("Match explicit output paths, schemas, field names, labels");
    expect(prompt).toContain("entity label and every required fact together");
    expect(prompt).toContain("as-of date supported by the records");
    expect(prompt).toContain("four tool-call rounds on inspection");
    expect(prompt).toContain("placeholder, TODO, pending section");
    expect(prompt).toContain("helper scripts or source code");
    expect(prompt).toContain("When authoring a SKILL.md");
    expect(prompt).toContain("valid YAML frontmatter");
    ...
    expect(prompt).toContain("After the last mutation, run one focused acceptance pass");
    expect(prompt).toContain("Reuse existing successful evidence");
