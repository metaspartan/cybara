...
describe("plugin UI", () => {
  ...
  test("combines installed bundles, account apps, and MCP services", async () => {
    ...
    expect(page).toContain(".marketplace(pluginSearch.trim(), marketplacePage, 24, catalogFilter)");
    expect(page).toContain("pluginsApi.installMarketplace");
    expect(page).toContain("setMarketplaceRevision((revision) => revision + 1)");
    ...
    expect(page).toContain('aria-label="Marketplace pagination"');
    expect(page).toContain('aria-label="Previous marketplace page"');
    expect(page).toContain('aria-label="Next marketplace page"');
    expect(page).toContain("Page {marketplacePage} of {marketplacePageCount}");
