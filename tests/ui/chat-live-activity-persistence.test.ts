...
describe("Chat live activity persistence", () => {
  ...
  test("uses server event timestamps when appending live activities to preserve order", () => {
    ...
    expect(source).toMatch(
      /appendLiveActivity\(\s*phase,\s*text,\s*payload\.toolName,\s*eventTimestamp,\s*payload\.toolCallId,\s*payload\.sandboxProvider,\s*imageSourceFromPath\(payload\.imagePath\),\s*imageAltFromPath\(payload\.imagePath\),\s*payload\.runId,\s*payload\.sequence,\s*payload\.fullDetail\s*\);/
    );
  });
