...
describe("IDE path allow-check symlink resolution", () => {
  afterEach(() => {
  ...
  test("a symlink under $HOME pointing outside $HOME resolves and is allowed", () => {
    workDir = mkdtempSync(join(homedir(), ".cybara-ide-test-"));
    const link = join(workDir, "escape");
    symlinkSync("/etc/passwd", link);
    expect(isPathAllowed(link)).toBe(true);
  });

  test("a not-yet-created file inside a symlinked directory is allowed", () => {
    workDir = mkdtempSync(join(homedir(), ".cybara-ide-test-"));
    const linkedDir = join(workDir, "outdir");
    symlinkSync("/etc", linkedDir);
    const wouldWrite = join(linkedDir, "newfile.txt");
    expect(isPathAllowed(wouldWrite)).toBe(true);
  });
