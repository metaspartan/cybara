...
describe("Security Manager", () => {
  ...
  describe("buildChannelSecurityConfig", () => {
    test("should default dm_policy to pairing when value is missing", () => {
      const result = buildChannelSecurityConfig({});
      expect(result.dm_policy).toBe("pairing");
      expect(Object.prototype.hasOwnProperty.call(result, "allowed_senders")).toBe(false);
    });

    test("should preserve valid dm_policy and sanitize allowed senders", () => {
      const result = buildChannelSecurityConfig({
        dm_policy: "allowlist",
        allowed_senders: ["user-1", "  user-2  ", "", 123, null],
      } as unknown as Record<string, unknown>);

      expect(result.dm_policy).toBe("allowlist");
      expect(result.allowed_senders).toEqual(["user-1", "user-2"]);
    });

    test("should ignore invalid dm_policy values", () => {
      const result = buildChannelSecurityConfig({
        dm_policy: "invalid-policy",
      });
      expect(result.dm_policy).toBe("pairing");
    });
  });
