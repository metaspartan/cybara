...
import { loadAllSkills, createEligibilityContext, filterEligibleSkills } from "../core/skills";
...
async function activeAgentSystemPrompt(
  agent: Pick<Agent, "id" | "name" | "model" | "tools" | "config" | "system_prompt">,
  workspaceDir?: string | null
): Promise<string> {
...
async function applyActiveAgentToSession(
...
async function refreshSessionAgentSystemPromptIfNeeded(
  session: InMemoryChatSession,
  agent: Pick<Agent, "id" | "name" | "model" | "tools" | "config" | "system_prompt">
): Promise<void> {
  const firstMessage = session.messages[0];
