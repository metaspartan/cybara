...
async function listOllamaModels(): Promise<string[]> {
  try {
    ...
    const names = (data.models || []).map((model) => model.name?.trim() || "").filter(Boolean);
...
async function primeOnnxRuntimeGlobal(): Promise<string | null> {
  ...
  onnxRuntimePrimeError =
    failures.join(" | ") || "Unable to initialize ONNX runtime backend (node/web)";
...
async function importOnnxRuntimeWebFallback(): Promise<Record<string, unknown>> {
  ...
  const fileNames = ["ort.node.min.mjs", "ort.all.min.mjs", "ort.bundle.min.mjs", "ort.min.mjs"];
...
function resolveTransformersModule(mod: Record<string, unknown>): TransformersModuleShape {
  ...
  for (const candidate of candidates) {
    ...
    const pipeline = resolvePipelineExport(candidate.pipeline) || resolvePipelineExport(candidate);
    if (pipeline) {
      return {
        pipeline,
        env:
...
async function importTransformersWebFallback(): Promise<Record<string, unknown>> {
  const candidates = collectTransformersDistCandidates(["transformers.web.js", "transformers.js"]);
...
function collectTransformersDistCandidates(fileNames: string[]): string[] {
  ...
  for (const root of roots) {
    for (const fileName of fileNames) {
      candidates.push(join(root, "node_modules", "@huggingface", "transformers", "dist", fileName));
    }
  }
...
async function importTransformersModule(): Promise<TransformersModuleShape> {
  ...
  throw new Error(`Unable to load Transformers.js runtime. Attempts: ${failures.join(" | ")}`);
}
...
function createOpenAIProvider(apiKey: string, modelInput?: string): EmbeddingProvider {
  const model = OPENAI_MODELS[modelInput || ""]
    ? modelInput || OPENAI_DEFAULT_MODEL
...
function createGeminiProvider(apiKey: string, modelInput?: string): EmbeddingProvider {
  const model = GEMINI_MODELS[modelInput || ""]
    ? modelInput || GEMINI_DEFAULT_MODEL
    ...
    if (transformersModule.env && typeof transformersModule.env === "object") {
      try {
        ...
        if (
          ...
          typeof transformersModule.env.backends === "object"
        ) {
  ...
  if (provider === "transformers_js") {
    if (!(await isTransformersJsAvailable())) {
      return {
        provider: createNullProvider(),
        source: "none",
        fallbackReason: transformersAvailabilityError || "Transformers.js package is not installed",
      };
    }
...
export async function getEmbeddingProviderCatalog(
  selection?: EmbeddingSelection
    ...
    providers: [
      ...
      },
      {
        ...
        reason: ollamaAvailable
          ? undefined
          : "Ollama is not running or no embedding models are installed",
...
export function getTransformersRuntimeStatus(
  selectedModelInput?: string
): TransformersRuntimeStatus {
  const selectedModel =
    selectedModelInput && selectedModelInput.trim()
  ...
  const loadedModels = models
    ...
    .sort((left, right) => {
      ...
      const leftWeight =
        left.state === "ready" ? 0 : left.state === "loading" ? 1 : left.state === "error" ? 2 : 3;
      const rightWeight =
        right.state === "ready"
          ? 0
          : right.state === "loading"
...
export async function loadEmbeddingRuntime(
  selection?: EmbeddingSelection
): Promise<EmbeddingRuntimeLoadResult> {
  const normalized = normalizeSelection(selection);
  ...
  if (provider === "transformers_js") {
    ...
    if (!(await isTransformersJsAvailable())) {
      ...
      return {
        ...
        model: selectedModel,
        message: transformersAvailabilityError || "Transformers.js package is not installed",
      };
    }
...
export async function stopEmbeddingRuntime(
  selection?: EmbeddingSelection
): Promise<EmbeddingRuntimeStopResult> {
  const normalized = normalizeSelection(selection);
  const provider = normalized.provider;
  const model = normalized.model;
  ...
  if (provider === "transformers_js") {
    ...
    if (!selectedModel) {
      const failures: string[] = [];
      const models = [
        ...new Set([...transformersExtractorCache.keys(), ...transformersRuntimeState.keys()]),
      ];
      for (const loadedModel of models) {
        try {
          ...
          failures.push(
            `${loadedModel}: ${error instanceof Error ? error.message : String(error)}`
          );
        }
      }
