...
export async function resolveRefToElement(
    ...
    const roleSelectors: Record<string, string> = {
        ...
        textbox: 'input[type="text"], input[type="search"], input[type="email"], input[type="url"], input[type="tel"], input:not([type]), textarea, [role="textbox"], [contenteditable="true"]',
    ...
    try {
        ...
        if (name) {
            ...
            for (let i = 0; i < elements.length; i++) {
                ...
                `) as { text: string; ariaLabel: string; title: string; placeholder: string; value: string };
                ...
                if (matchesName) {
                    ...
                    if (nth !== undefined && nth > 0) {
                        ...
                        for (let j = 0; j <= i; j++) {
                            ...
                            if (checkProps.text === name || checkProps.ariaLabel === name ||
                                checkProps.text.includes(name) || checkProps.ariaLabel.includes(name)) {
                                matchCount++;
                            }
                        }
...
export async function waitFor(
    ...
    if (options.url) {
        ...
        await page.waitForFunction(
            `window.location.href.includes(${escapedUrl})`,
            { timeout }
        );
    }
