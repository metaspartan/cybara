...
import { withCronLock } from "./file-lock";

const CRON_DIR = join(cybaraDir, "cron");
...
export function updateJob(id: string, patch: CronJobPatch): CronJob | null {
  return withCronLock("jobs", () => {
    ...
    if (patch.state) Object.assign(job.state, patch.state);
...
export function saveRunLogs(logs: CronRunLog[]): void {
  withCronLock("runs", () => writeJsonAtomic(RUNS_FILE, logs.slice(-MAX_RUN_LOGS)));
}
...
export function addRunLog(log: CronRunLog): void {
  withCronLock("runs", () => {
    ...
    logs.push(log);
    writeJsonAtomic(RUNS_FILE, logs.slice(-MAX_RUN_LOGS));
  });
}
