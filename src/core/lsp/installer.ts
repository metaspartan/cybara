...
export const LSP_REGISTRY: Record<string, LSPInfo> = {
    ...
    zig: {
        ...
        downloadUrls: {
            ...
            linux_x64:
                "https://github.com/zigtools/zls/releases/latest/download/zls-x86_64-linux.tar.xz",
            linux_arm64:
    ...
    if (url.endsWith(".gz") && !url.endsWith(".tar.gz")) {
        ...
        try {
            if (url.endsWith(".zip")) {
                ...
                const result = Bun.spawnSync(["unzip", "-o", archivePath, "-d", extractDir], { timeout: 60000 });
                ...
                // Extract tar.xz
                const result = Bun.spawnSync(["tar", "-xJf", archivePath, "-C", extractDir], { timeout: 60000 });
                if (result.exitCode !== 0) {
                    unlinkSync(archivePath);
                ...
                // Extract tar.gz
                const result = Bun.spawnSync(["tar", "-xzf", archivePath, "-C", extractDir], { timeout: 60000 });
                if (result.exitCode !== 0) {
                    unlinkSync(archivePath);
            ...
            const findBinary = Bun.spawnSync(["find", extractDir, "-name", info.binaryName, "-type", "f"], { timeout: 30000 });
