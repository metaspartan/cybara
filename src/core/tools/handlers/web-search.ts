...
const USER_AGENT = "Mozilla/5.0 (Macintosh; Intel Mac OS X 14_7_2) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.0.0 Safari/537.36";
...
// Brave Search API
async function searchWithBrave(query: string, count: number, apiKey: string): Promise<SearchResponse> {
...
export async function handleWebSearch(
    ...
    const count = Math.min(Math.max(1, (args.count as number) || DEFAULT_SEARCH_COUNT), MAX_SEARCH_COUNT);
    ...
    try {
        ...
        if (braveApiKey) {
            try {
                ...
                throw new Error(`All search providers failed: ${(error as Error).message}, DDG: ${(fallbackError as Error).message}`);
            }
        }
        throw error;
    }
