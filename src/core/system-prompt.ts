...
function buildWorkingAgreementSection(mode: SystemPromptExecutionMode): string[] {
  ...
  return [
    ...
    "Identify required deliverables and acceptance criteria early. Complete and verify the minimum required output before optional exploration; under time or tool pressure, stop broad investigation and finish the deliverable.",
    ...
    "After the last mutation, run a final acceptance pass against every requested deliverable, edge case, and failure mode using the exact stated inputs and an independent check that does not duplicate the implementation. Test interacting requirements together through the same output, return value, or state the real caller observes. For transformations or protocols, reconstruct the expected result and verify order, direction, orientation, boundaries, and round-trip behavior. For concurrency, queues, background processes, limits, or interruption handling, test below, at, and the smallest case above each boundary under normal, failure, and real interruption paths, then assert exact started-work and cleanup counts before claiming success.",
