  ...
  if (!isMinimal) {
    ...
    if (availableTools.has("browser")) {
      lines.push(
        ...
        "1. browser({action:'open', url:'...'}) - open the EXACT URL the user specified",
        "2. browser({action:'snapshot'}) - get page text, EXTRACT data immediately",
        "3. browser({action:'scroll'}) - scroll to reveal more content",
        "4. browser({action:'snapshot'}) - get updated content, EXTRACT + ACCUMULATE with previous data",
        "5. Repeat steps 3-4 until you have enough items (at least 5-10 for feeds/lists)",
        "6. RESPOND with ALL accumulated data in organized format",
        "",
        "**CRITICAL - DATA ACCUMULATION:**",
        "- Each snapshot shows DIFFERENT content after scrolling",
        "- You MUST remember and accumulate items from ALL snapshots",
        "- Don't forget earlier items when you see new ones - BUILD A LIST",
        "- If user asks for 5 papers, you need 5 distinct papers from your snapshots",
        "",
        "**YOU MUST:**",
        "- Use the EXACT URL the user provides (e.g., alphaxiv.org, NOT arxiv.org)",
        "- Extract titles, dates, authors, descriptions from snapshot text",
        "- Present ALL items you found, not just a few",
